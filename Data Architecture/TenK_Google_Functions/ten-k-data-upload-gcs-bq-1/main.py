...
def get_and_upload_api_data(request):
    ...
    api_data = [
        ...
        {
            "url": "https://api.rm.smartsheet.com/api/v1/assignables",
            "bucket_name": "method-incoming",
            "file_name": f"10k/assignables/10k_assignables_data_{datetime.now().strftime('%d%m%Y_%H%M%S')}.jsonl",
            "params": {"per_page": 100, "page": 1},
            "dataset_name": "Landing",
            "table_name": "TenK_Assignables"
        },
        # Assignment Data API Call
        {
            "url": "https://api.rm.smartsheet.com/api/v1/assignments",
            "bucket_name": "method-incoming",
            "file_name": f"10k/assignments/10k_assignments_data_{datetime.now().strftime('%d%m%Y_%H%M%S')}.jsonl",
            "params": {
                "from": "2020-01-01",
                "to": "2023-12-31",
                "per_page": 100,
            ...
            "dataset_name": "Landing",
            "table_name": "TenK_Assignments"
        },
        # Projects Data API Call
        {
            "url": "https://api.rm.smartsheet.com/api/v1/projects",
            "bucket_name": "method-incoming",
            "file_name": f"10k/projects/10k_projects_data_{datetime.now().strftime('%d%m%Y_%H%M%S')}.jsonl",
            "params": {
                "from": "2020-01-01",
                "to": "2023-12-31",
                "per_page": 100,
            ...
            "dataset_name": "Landing",
            "table_name": "TenK_Projects"
        },
        # Users Data API Call
        {
            "url": "https://api.rm.smartsheet.com/api/v1/users",
            "bucket_name": "method-incoming",
            "file_name": f"10k/users/10k_users_data_{datetime.now().strftime('%d%m%Y_%H%M%S')}.jsonl",
            "params": {
                "per_page": 100,
                "page": 1,
                "fields": "tags,custom_field_values",
                "with_archived": True,
                "tags.paging.page": 1,
                "custom_field_values.paging.page": 1
            },
            "dataset_name": "Landing",
            "table_name": "TenK_Users"
        }
    ]

    audit_data = []
    # Running the loop for all the API calls, fetch the data, put them into GCS and then to BigQuery
    for api in api_data:
        # Add run names of each table
        run_name = f'API_Name_{api["table_name"]}'
        file_destination = api['file_name']
        # Calling the API
        data = get_api_data(api['url'], headers=headers, params=api['params'])
        
        # Adding row count of each table
        rows_count = len(data)
        ...
        row_count_BQ = upload_to_bigquery(data, api['dataset_name'], api['table_name'])
        ...
        # Perform the audit for the current API call
        audit_table_name = "TenK_Audit"  # Name of the audit table
        audit_dataset_name = "Audit"  # Name of the audit dataset

        audit_data.append({
            "run_name": run_name,
            "start_time": start_time,
            "end_time": datetime.utcnow().replace(microsecond=0).isoformat() + 'Z',
            "Data_Retrieved": rows_count,
            "Data_Uploaded": row_count_BQ,
            "file_path_source": file_destination,
        })

    # Upload the updated audit data to the audit table in BigQuery
    upload_to_bigquery(audit_data, audit_dataset_name, audit_table_name)
    ...
    def publish_message(project_id, topic_id, message):
        ...
        # Create a PubsubMessage object
        pubsub_message = pubsub_v1.types.PubsubMessage(data=message_bytes)

        # Publish the message to the topic
        future = publisher.publish(topic_path, data=pubsub_message.data)
